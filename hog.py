...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    i, outcome, flag = 0, 0, 0
    while i < num_rolls:
        temp = dice()
        if temp == 1 :
            flag = 1
        outcome, i = outcome + temp, i + 1
    if flag == 1 :
        outcome = flag
    return outcome
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opponent_second_digit = (opponent_score // 10) % 10
    player_first_digit = player_score % 10
    score = abs(opponent_second_digit - player_first_digit)
    if score == 0 :
        return 1
    else :
        return 3*score
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_score = averaged_roll_dice(1, dice)
    i, num = 2, 1
    while i <= 10 :
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if updated_score - current_score >= threshold :
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** If the goal score can be surpassed by 0,1,2 dice, then return that number.
        If rolling 0 gives a higher score on average than rolling 6, return 0. ***
    """
    # BEGIN PROBLEM 12
    goal = 100
    updated_score0 = sus_update(0, score, opponent_score)
    updated_score1 = sus_update(1, score, opponent_score)
    updated_score2 = sus_update(2, score, opponent_score)
    if updated_score0 >= goal :
        return 0
    if updated_score1 >= goal :
        return 1
    if updated_score2 >= goal :
        return 2
    always_roll_6 = always_roll(6)
    if updated_score0 - score >= always_roll_6(score, opponent_score) :
        return 0
